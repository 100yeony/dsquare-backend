...
public class MypageController {
    ...
    public List<BriefQuestionResponse> getAllMyQuestions(@AuthUser Member user, String order, Pageable pageable){
        return mypageService.getAllMyQuestions(user, order, pageable);
    }

    //read - 나의 카드주세요 글 전체 조회
    @GetMapping("/mypage/cards")
    public List<BriefCardResponse> getAllMyCards(@AuthUser Member user, String order, Pageable pageable){
        return mypageService.getAllMyCards(user, order, pageable);
    }

    //read - 나의 소통해요 글 전체 조회
    @GetMapping("/mypage/talks")
    public List<BriefTalkResponse> getAllMyTalks(@AuthUser Member user, String order, Pageable pageable){
        return mypageService.getAllMyTalks(user, order, pageable);
    }

    //read - 나의 당근해요 글 전체 조회
    @GetMapping("/mypage/carrots")
    public List<BriefCarrotResponse> getAllMyCarrots(@AuthUser Member user, String order, Pageable pageable){
        return mypageService.getAllMyCarrots(user, order, pageable);
    }

    //read - 나의 답변글 전체 조회
    @GetMapping("/mypage/answers")
    public List<AnswerResponse> getAllMyAnswers(@AuthUser Member user, String order, Pageable pageable){
        return mypageService.getAllMyAnswers(user, order, pageable);
    }

    //read - 나의 댓글 전체 조회
    @GetMapping("/mypage/comments")
    public List<MyCommentInfo> getAllMyComments(@AuthUser Member user, String order, Pageable pageable){
        return mypageService.getAllMyComments(user, order, pageable);
    }
}
